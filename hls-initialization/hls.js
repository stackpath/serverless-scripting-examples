...
function buildModificationConfig(deviceData) {
  ...
  if (
    (deviceData.device_os === 'iOS' && parseInt(deviceData.device_os_version, 10) < 7)
    || (deviceData.device_os === 'Android' && parseInt(deviceData.device_os_version, 10) < 6)
...
function buildNewPlaylist(qualities, config) {
  ...
  if (config.capAtResolution) {
    ...
    if (newQualities.length > 0) {
      filteredQualities = newQualities;
    }
  }
  ...
  filteredQualities.sort((a, b) => ((a.bitrate > b.bitrate) ? -1 * dir : dir));
  ...
  if (config.qualityPriority === HIGHEST_WITHIN_RESOLUTION) {
    ...
    for (i in filteredQualities) {
      topQuality = filteredQualities[i];

      // Separate dimensions into width and height and take the higher of the
      // two.
      const dimension = filteredQualities[i].resolution.split('x').sort()[1];
      ...
      if (dimension <= config.resolution) {
        ...
        // quality.
        if (filteredQualities[i].bitrate < 4000000) {
          break;
        }

        // Otherwise, set the top quality to the next one, if it exists.
        if (filteredQualities.length > i) {
          i += 1;
          topQuality = filteredQualities[i];
          break;
        }
    ...
    filteredQualities.splice(0, 0, topQuality);
  }

  // Otherwise, if the middle quality is preferred then move it to the top of
  // the list.
  if (config.qualityPriority === MIDDLE_FIRST) {
    const m = Math.floor(filteredQualities.length / 2);
    const middleItem = filteredQualities[m];
    filteredQualities.splice(m, 1);
    filteredQualities.splice(0, 0, middleItem);
  }
...
async function handleRequest(request) {
  try {
    ...
    const [openDdrResponse, originResponse] = await Promise.all([
